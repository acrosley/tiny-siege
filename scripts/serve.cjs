...
const root = path.resolve(__dirname, ".."),
  DB = openDb(path.join(root, ".data/rooms.sqlite"));
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname.startsWith("/api/")) {
      ...
      const response = await (await handler).api(request, { DB });
...
server.listen(process.env.PORT || 4173, process.env.HOST || "127.0.0.1", () =>
  console.log(`Tiny Siege: http://127.0.0.1:${process.env.PORT || 4173}`),
);
